...
PYTHON := python3
COVERAGE := --cov=dc_qiskit_algorithms --cov-report term-missing --cov-report=html:coverage_html_report
TESTRUNNER := -m pytest tests

.PHONY: help
help:
	@echo "Please use \`make <target>' where <target> is one of"
	@echo "  install            to install dc-qiskit-algorithms"
	@echo "  wheel              to build the dc-qiskit-algorithms wheel"
...
install:
ifndef PYTHON3
	@echo "To install dc-qiskit-algorithms you need to have Python 3 installed"
endif
...
.PHONY : clean
clean:
	rm -rf dc_qiskit_algorithms/__pycache__
